    ...
    if (refresh_token2) {
        if (ADrivreInfo.refresh_token) {
            if (ADrivreInfo.refresh_token !== refresh_token2) {
                ...
                if (t) {
                    $.msg('refresh refresh_token successful 🎉', '', '')
                } else {
                    $.msg('refresh refresh_token failure‼️', '', '')
                }
            }
        } else {
            ADrivreInfo.refresh_token_body = body
            ...
            let t = $.setjson(ADrivreInfo,keyName)
            if (t) {
                $.msg('first time get refresh_token successful 🎉', '', '')
            } else {
                $.msg('first time get refresh_token failure‼️', '', '')
            }
        }
    }
    $.done()
...
function getAuthorizationKey() {
    ...
    $.log('get authorization')
    $.post(option, function (error, response, data) {
        if (error) {
            $.log('error：' + error)
            $.msg(title, '❌checkin failure', 'refresh authorization failure')
            return $.done()
        } else if(!data)
        {
            $.log('Nodata')
            ...
            if (refresh_token2) {
                ...
                let t = $.setjson(ADrivreInfo,keyName)
                if (t) {
                    $.log('refresh refresh_token successful 🎉')
                } else {
                    $.msg('refresh refresh_token failure‼️', '', '')
                }
            }
...
function signCheckin(authorization) {
    ...
    $.post(url_fetch_sign, function (error, response, data) {
        if (error) {
            $.log('error：' + error)
            $.msg(title, '❌Checkin failure', 'can't checkin, please checkin manually')
            $.done()
        } else if(!data)
        {
            $.log('no data')
            ...
            let stitle = '🎉' + body.result.title + ' checkin successful'
