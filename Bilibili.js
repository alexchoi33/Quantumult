    ...
    switch (!0) {
        case /pgc\/season\/app\/related\/recommend\?/.test($request.url):
            try {
                ...
                a.result?.cards?.length && (a.result.cards = a.result.cards.filter(a => 2 != a.type)),
        ...
        case /^https?:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\/story\?/.test($request.url):
            try {
                let a = JSON.parse(body),
                    b = [];
                for (let c of a.data.items)
                    c.hasOwnProperty("ad_info") || -1 !== c.card_goto.indexOf("ad") || b.push(c);
                a.data.items = b,
                    body = JSON.stringify(a)
            } catch (a) {
            ...
            try {
                ...
                a.data.sections_v2.forEach((c, d) => {
                    ...
                    a.data.sections_v2[d].items = e,
                        ...
                        ("\u521B\u4F5C\u4E2D\u5FC3" == a.data.sections_v2[d].title || "\u5275\u4F5C\u4E2D\u5FC3" == a.data.sections_v2[d].title) && (delete a.data.sections_v2[d].title, delete a.data.sections_v2[d].type)
                }),
                    ...
                    a.data.vip.status || (a.data.vip_type = 2, a.data.vip.type = 2, a.data.vip.status = 1, a.data.vip.vip_pay_type = 1, a.data.vip.due_date = 466982416e4),
                    body = JSON.stringify(a)
            } catch (a) {
        ...
        case /^https?:\/\/api\.live\.bilibili\.com\/xlive\/app-room\/v1\/index\/getInfoByRoom/.test($request.url):
            try {
                ...
                a.data.activity_banner_info = null,
                    a.data?.shopping_info && (a.data.shopping_info = {
                        is_show: 0
                    }),
                    a.data?.new_tab_info?.outer_list && a.data.new_tab_info.outer_list.length && (a.data.new_tab_info.outer_list = a.data.new_tab_info.outer_list.filter(a => 33 != a.biz_id)),
                    body = JSON.stringify(a)
            } catch (a) {
        ...
        case /https?:\/\/app\.bilibili\.com\/x\/v2\/account\/myinfo\?/.test($request.url):
            try {
                let a = JSON.parse(body);
                a.data.vip.status || (a.data.vip.type = 2, a.data.vip.status = 1, a.data.vip.vip_pay_type = 1, a.data.vip.due_date = 466982416e4),
                    body = JSON.stringify(a)
            } catch (a) {
        ...
        case /pgc\/page\/(bangumi|cinema\/tab\?)/.test($request.url):
            try {
                ...
                a.result.modules.forEach(a => {
                    a.style.startsWith("banner") && (a.items = a.items.filter(a => -1 != a.link.indexOf("play"))),
                        a.style.startsWith("function") && (a.items = a.items.filter(a => -1 == a.blink.indexOf("bilibili.com")), [1283, 241, 1441, 1284].includes(a.module_id) && (a.items = [])),
        ...
        case /^https:\/\/api\.live\.bilibili\.com\/xlive\/app-interface\/v2\/index\/feed/.test($request.url):
            try {
                let a = JSON.parse(body);
                a.data && a.data.card_list && (a.data.card_list = a.data.card_list.filter(a => "banner_v1" != a.card_type)),
                    body = JSON.stringify(a)
            } catch (a) {
